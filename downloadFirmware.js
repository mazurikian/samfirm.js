...
const downloadFirmware = async (model, region, imei) => {
  try {
    console.log(`Fetching Firmware for
      Model: ${model}
      Region: ${region}
      IMEI: ${imei}`);

    const { pda, csc, modem } = await getLatestFirmwareVersion(region, model);
    console.log(`Latest Firmware Versions
      PDA: ${pda}
      CSC: ${csc}
      MODEM: ${modem}`);
...
downloadFirmware(argv.model, argv.region, argv.imei);
